...
export const chinaRegions: Region[] = [
  ...
  },
  {
    ...
    "children": [
      ...
      { "code": "360200", "name": "九江市" },
      { "code": "360300", "name": "景德镇市" },
      { "code": "360400", "name": "萍乡市" },
      { "code": "360500", "name": "新余市" },
      { "code": "360600", "name": "鹰潭市" },
      { "code": "360700", "name": "赣州市" },
      { "code": "360800", "name": "宜春市" },
      { "code": "360900", "name": "上饶市" },
      { "code": "361000", "name": "吉安市" },
      { "code": "361100", "name": "抚州市" }
    ]
  },
  {
    "code": "370000",
    ...
    "children": [
      {
        ...
        "children": [
          ...
          { "code": "500157", "name": "城口县" },
          { "code": "500158", "name": "丰都县" },
          { "code": "500159", "name": "垫江县" },
          { "code": "500160", "name": "忠县" },
          { "code": "500161", "name": "云阳县" },
          { "code": "500162", "name": "奉节县" },
          { "code": "500163", "name": "巫山县" },
          { "code": "500164", "name": "巫溪县" },
          { "code": "500165", "name": "石柱土家族自治县" },
          { "code": "500166", "name": "秀山土家族苗族自治县" },
          { "code": "500167", "name": "酉阳土家族苗族自治县" },
          { "code": "500168", "name": "彭水苗族土家族自治县" }
        ]
      }
    ]
  },
  ...
  },
  {
    ...
    "children": [
      ...
      { "code": "710300", "name": "新北市" },
      { "code": "710400", "name": "台中市" },
      { "code": "710500", "name": "台南市" },
      { "code": "710600", "name": "桃园市" },
      { "code": "710700", "name": "宜兰县" },
      { "code": "710800", "name": "新竹县" },
      { "code": "710900", "name": "苗栗县" },
      { "code": "711000", "name": "彰化县" },
      { "code": "711100", "name": "南投县" },
      { "code": "711200", "name": "云林县" },
      { "code": "711300", "name": "嘉义县" },
      { "code": "711400", "name": "屏东县" },
      { "code": "711500", "name": "台东县" },
      { "code": "711600", "name": "花莲县" },
      { "code": "711700", "name": "澎湖县" },
      { "code": "711800", "name": "金门县" },
      { "code": "711900", "name": "连江县" }
    ]
  },
  {
    "code": "810000",
...
// 根据父级code获取子级区域
export const getChildRegions = (parentCode: string): Region[] => {
  if (!parentCode) {
    return chinaRegions;
  }
  
  const findRegion = (regions: Region[], code: string): Region | undefined => {
  ...
  const region = findRegion(chinaRegions, parentCode);
...
export const getRegionName = (code: string): string => {
  ...
  const region = findRegion(chinaRegions, code);
  return region?.name || '';
};

// 根据code获取完整区域路径（如：重庆市/重庆市/渝北区）
export const getRegionPath = (code: string): string => {
  const findPath = (regions: Region[], code: string, path: string[] = []): string[] | null => {
    for (const region of regions) {
      if (region.code === code) {
        return [...path, region.name];
      }
      if (region.children) {
        const result = findPath(region.children, code, [...path, region.name]);
        if (result) {
          return result;
        }
      }
  ...
  const path = findPath(chinaRegions, code);
  return path ? path.join('/') : '';
};

// 搜索区域
export const searchRegions = (keyword: string): Region[] => {
  const results: Region[] = [];
  
  const search = (regions: Region[]) => {
    for (const region of regions) {
      if (region.name.includes(keyword)) {
        results.push(region);
      }
      if (region.children) {
        search(region.children);
      }
    }
  };
